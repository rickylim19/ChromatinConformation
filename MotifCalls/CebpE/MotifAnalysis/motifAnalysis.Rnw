...
<<biclusteringLog, cache=TRUE>>=
# filter out the clusters with single peaks only
Koeffler_BM_CebpE_BiclusterSinglePeakFilteredOut_NBM_comp4_log_dist3kb <-
 assignBiclustering(paste0(work_dir,
    'Input/Koeffler_BM_CebpE_NBM_ModelAssignment_compSorted4.bed'),
    distance = 3000, filterOutSingle = TRUE, 
    paste0(work_dir,
...
\section{Centdist Motif Results for Log-Transformed ChIPseq for Group1 and Group4}

\begin{figure}[H]
    \centering
    \subfloat{\includegraphics[width = 6in, height = 6in]{/home/ricky/Rlim/ChromatinConformation/ComponentCalls/CebpE/ComponentAnalysis/figs/Koeffler_BM_CebpE_BoxplotProfile.png}}
...
\subsection{Motif ChipSeq Distribution of CEBPB and CTCF}
<<>>=
...
\subsection{Motif ChIPseq Distribution per Groups}
<<>>=
...
<<ctcfCebp, results='asis', echo=FALSE>>=
